...
-- ENT.Category = "Brainz"
-- ENT.Author = "Ethorbit, Lolle & Zet0r"
-- ENT.Spawnable = true
-- ENT.AdminOnly = true
...
-- 	self:SetHealth( 999999999 ) --fallback
...
-- 					self:AttachToClosestZombie() -- If we are stuck then let's just spawn to the closest zombie, we cannot despawn ourselves!
-- 					--self:RespawnZombie()
-- 					self:SetStuckCounter( 0 )
-- 				end
...
-- 	end
-- 	if GetConVar( "nz_zombie_debug" ):GetBool() then
-- 		-- EDIT: We won't want to confuse developers by showing Anti-Cheat ghost's debug lines too, they're only interested in REAL zombie's pathwork
...
-- 	-- EDIT: Anti-Cheat zombies should NEVER interact with barricades
...
-- 	-- EDIT: AntiCheat Zombies should NEVER get injured
...
-- 	-- EDIT: AntiCheat Zombies should NEVER get killed
...
-- function ENT:GotoNearestNavmesh() -- Go to the closest navmesh to our enemy to determine if they are cheating
-- 	if (!IsValid(self:GetTarget())) then return end
-- 	--local nav = navmesh.GetNearestNavArea(self:GetTarget():GetPos())
-- 	local navs = navmesh.GetAllNavAreas()
-- 	table.sort(navs, function(a, b) 
-- 		local closer = a:GetCenter():DistToSqr(self:GetTarget():GetPos()) < b:GetCenter():DistToSqr(self:GetTarget():GetPos())
-- 		local locked = nzNav.Locks[a:GetID()]
-- 		return closer and !locked
-- 	end)

-- 	if (!istable(navs)) then return end
-- 	local nav = navs[1]
-- 	if IsValid(nav) then
...
-- function ENT:AttachToClosestZombie() -- Mimics the closest zombie to our target
-- 	if (IsValid(self:GetTarget())) then
-- 		--self:GotoNearestNavmesh()
-- 		local anyZombies = #ents.FindByClass("nz_zombie_walker")
-- 		if (!anyZombies) then -- Maybe the round is over?
-- 			timer.Simple(1, function() -- Keep trying until a zombie exists
...
-- 		for k,v in pairs(ents.FindByClass("nz_zombie_walker")) do
-- 			local newDist = v:GetPos():DistToSqr(self:GetTarget():GetPos())
-- 			if (closestDist == nil || newDist < closestDist) then
-- 				closestPos = v:GetPos()
-- 				closestDist = newDist
-- 				closestZombie = v
-- 			end
...
-- 		if (IsValid(closestZombie)) then
-- 			self:SetWalkSpeed(closestZombie:GetWalkSpeed())
-- 			self:SetRunSpeed(closestZombie:GetRunSpeed())
-- 			self:SetPos(closestZombie:GetPos())
-- 			self.ZombieParent = closestZombie
-- 		end
-- 	end
-- end

-- function ENT:GetZombieParent()
-- 	return self.ZombieParent
-- end
...
-- 		self.onavalidpath = false

-- 		if (IsValid(self:GetZombieParent()) and self:GetTarget() and IsValid(self:GetTarget())) then -- We're not quite high enough to reach them!
-- 			if (self:GetTarget():GetVelocity()[3] == 0) then -- They are higher than our height
-- 				self:SetPos(self:GetZombieParent():GetPos())
-- 				--timer.Simple(0.3, function()
-- 					if (!self.onavalidpath) then -- We've been unable to follow the path for a bit now
-- 						-- If they truly are cheating then all other zombies would have stopped targetting them by now:
-- 						local targetAmount = 0
-- 						for k,v in pairs(ents.FindByClass("nz_zombie_walker")) do
-- 							if (#player.GetAllPlayingAndAlive() == 1 and v:IsTimedOut() or #player.GetAllPlayingAndAlive() > 1) then
-- 								targetAmount = targetAmount + 1
-- 							end
-- 						end

-- 						for k,v in pairs(ents.FindByClass("nz_zombie_walker")) do
-- 							if (#player.GetAllPlayingAndAlive() > 1 and v:GetTarget() == self:GetTarget()) then
-- 								targetAmount = targetAmount + 1
-- 							end
-- 						end
...
-- 						if (targetAmount) then -- No zombies can target them
-- 							if (self:GetTarget():CanBeCheater()) then 
-- 								self:GetTarget():OnTargetOutOfReach()
-- 							end 
-- 						end
-- 					end
-- 				--end)
