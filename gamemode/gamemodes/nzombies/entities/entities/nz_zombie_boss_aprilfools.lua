...
ENT.PrintName = "Octagonal Robber"
ENT.Category = "Brainz"
ENT.Author = "Laby"

ENT.Models = { "models/roach/bo1_overhaul/thief.mdl"}
...
ENT.WalkSounds = {
"bo1_overhaul/thief/amb1.mp3",
"bo1_overhaul/thief/amb2.mp3",
"bo1_overhaul/thief/amb3.mp3"
}
...
function ENT:InitDataTables()
	...
	self:NetworkVar("String", 0, "Stolenwep")
	self:NetworkVar("Entity", 1, "muggedPlayer")
end
...
function ENT:OnSpawn()
	...
	if coroutine.running() then
		...
		local pos = self:GetPos() + (seq == "taunt1" and Vector(0,0,100) or Vector(0,0,450))
		for i=1,math.random(3,5) do
		timer.Simple(0.1*i,function()ParticleEffectAttach("thief_smog",PATTACH_ABSORIGIN_FOLLOW,self,0) end)
		end
	ParticleEffect("driese_tp_departure_phase2",self:GetPos(),self:GetAngles(),self)
	self:EmitSound("bo1_overhaul/thief/teleport.mp3",511)
		counting = true
...
function ENT:BodyUpdate()
	...
	if ( len2d > 5 ) then self.CalcIdeal = ACT_WALK elseif ( len2d > 600 ) then self.CalcIdeal = ACT_RUN end
			...
			for k,v in pairs(ents.FindInSphere(self:GetPos(),80)) do
					...
					if stuckplayer then
						...
						local run = stuckplayer:GetRunSpeed()
							stuckplayer:SetRunSpeed(1)
							stuckplayer:SetWalkSpeed(1)
							stuckplayer:Freeze(true)
						...
						local wep = stuckplayer:GetActiveWeapon():GetClass()
						stuckplayer:StripWeapon(wep)
						stuckplayer:Freeze(false)
