	...
	if math.random(0,5) == 0 and CurTime() > self.NextClawTime then
		...
		if self:IsValidTarget(target) then
			...
			if IsValid(tr.Entity) and self:IsValidTarget(tr.Entity) and !IsValid(self.ClawHook) then
				self:Stop()
				self:EmitSound("avogadro_taunt_"..math.random(12)..".mp3")
				self:PlaySequenceAndWait("range_in")
				--self:SetSequence(self:LookupSequence("nz_grapple_loop"))
				self:ResetSequence("range_loop")
				isBall = true
				self.loco:SetDesiredSpeed(500)
				self:SetLocalVelocity((((tr.Entity:GetPos() + Vector(0,0,50))* 500)
				self:SetAngles(((((tr.Entity:GetPos() + Vector(0,0,50))*-1):Angle())
...
if !isTaunting then
	...
	timer.Simple(5, function()isTaunting=true end)
end
...
function ENT:OnInjured( dmgInfo )
	...
	self:EmitSound("avogadro_damaged_"..math.random(12)..".mp3")
	end
