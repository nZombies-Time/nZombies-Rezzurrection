...
ENT.Type = "nextbot"
ENT.PrintName = "Baboon Xenomorph that headbutts you"
ENT.Category = "Brainz"
ENT.Author = "Wavy"
ENT.Spawnable = true
...
ENT.Models = {
	{Model = "models/wavy/wavy_enemies/xenomorph/brute/xenomorph_brute.mdl", Skin = 0, Bodygroups = {0,0}},
}
...
ENT.JumpAttackDamage = 90
ENT.JumpDamageRange = 125

ENT.IdleSequence = "nz_xeno_brute_idle"
...
ENT.LeapAttackSequences = {
	"nz_xeno_brute_charge",
	"nz_xeno_brute_leap_headbutt",
}
...
ENT.ChargeImpactSounds = {
	"character/alien/vocals/brute/charger_smash_01.wav",
	"character/alien/vocals/brute/charger_smash_02.wav",
	"character/alien/vocals/brute/charger_smash_03.wav",
}
...
function ENT:StatsInitialize()
	if SERVER then
		if nzRound:GetNumber() == -1 then
			...
			self:SetHealth( nzRound:GetZombieHealth() * 1.5 or 75 )
		end
		
		self:SetCollisionBounds(Vector(-22,-22, 0), Vector(22, 22, 74))
...
function ENT:PostAdditionalZombieStuff()
	...
	if self:TargetInRange(180) and !self:IsAttackBlocked() and self.CanJump then
...
function ENT:JumpAttack()
	...
	self:EmitSound("character/alien/vocals/brute/brute_charge"..math.random(1,3)..".ogg", 500, 100, 1, 2)
	...
	if IsValid(target) and target:Health() and target:Health() > 0 then -- Doesn't matter if its a player... If the zombie is targetting it, they probably wanna attack it.
		if self:TargetInRange( range ) then
			...
			if self:TargetInRange( dmgrange ) then
				...
				target:EmitSound(self.ChargeImpactSounds[math.random(#self.ChargeImpactSounds)], SNDLVL_TALKING, math.random(95,105))
			end
