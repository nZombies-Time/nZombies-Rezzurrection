...
ENT.Models = {
	{Model = "models/bosses/larry.mdl", Skin = 0, Bodygroups = {0,0}},
}
...
local walksounds = {
	Sound("enemies/bosses/larry/observe.ogg"),
	Sound("enemies/bosses/larry/partofthejob.ogg"),
	Sound("enemies/bosses/larry/BREATH.ogg"),
	Sound("enemies/bosses/larry/salad.ogg"),

}

ENT.AttackSounds = {
	"enemies/bosses/larry/hit1.ogg",
	"enemies/bosses/larry/hit2.ogg",
	"enemies/bosses/larry/hit3.ogg"
}
...
ENT.DeathSounds = {
	"enemies/bosses/larry/death.ogg"
}
...
function ENT:OnSpawn()
	...
	self:EmitSound("enemies/bosses/larry/hit1.ogg",511)
		...
		if not v:GetNotDowned()  then
			if  CurTime() > self.NextTeleporTime then
				...
				for k,v in pairs(player.GetAll()) do
					...
					self:EmitSound("enemies/bosses/larry/observe.ogg",511)
				...
				self:SetPos( pos )
				self:EmitSound("enemies/bosses/larry/partofthejob.ogg",511)
				self:EmitSound("enemies/bosses/thrasher/tele_hand_up.ogg",511)
				ParticleEffect("bo3_zombie_spawn",self:GetPos()+Vector(0,0,1),Angle(0,0,0),nil)
			...
			if actionchance == 2 and CurTime() > self.NextTeleporTime then
				...
				local pos = self:FindSpotBehindPlayer(target:GetPos(), 10)
				self:SetPos( pos )
				self:EmitSound("enemies/bosses/larry/BREATH.ogg",511)
				self:EmitSound("enemies/bosses/thrasher/tele_hand_up.ogg",511)
				ParticleEffect("bo3_zombie_spawn",self:GetPos()+Vector(0,0,1),Angle(0,0,0),nil)	
			...
			elseif not enraged and actionchance == 1 and CurTime() > self.NextAction then
				...
				self:EmitSound("enemies/bosses/larry/ANGY.ogg",511)
				ParticleEffect("bo3_astronaut_pulse",self:LocalToWorld(Vector(0,0,60)),Angle(0,0,0),nil)	
				self:PlaySequenceAndWait("nz_anger")
