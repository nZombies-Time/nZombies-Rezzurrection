-- Developed by Ethorbit to stop nZombie cheaters!
...
function PLAYER:ACSavePoint() -- Save the last position they were not cheating at for Anti-Cheat teleports
    ...
    if (self.allowsavespot and self.saveDelay == nil or CurTime() > self.saveDelay and self.allowsavespot) then
        self.saveDelay = CurTime() + 0.5 -- Optimization, don't need to save positions every tick
...
function PLAYER:IsInExcludedArea() -- These spots are manually added in spots that players can cheat in if they want to, zombies aren't supposed to be here
    if !NZCurConfig or !NZCurConfig["AntiCheatExclusions"] then return false end -- There is no exclusion area set for this map
    local returnVal = false

    for k,v in pairs(NZCurConfig["AntiCheatExclusions"]) do
        if !v["Position"] || !v["Radius"] then print("AntiCheatExclusions table in map config is incorrect! (" .. game.GetMap() .. ")") return end 
        if self:GetPos():Distance(v["Position"]) <= v["Radius"] then
            returnVal = true
            break
        end
    end
...
function PLAYER:NZMoveCheater() -- Teleports them out of the cheat spot
    ...
    if !self.Teleporting then
        ...
        timer.Simple(0.1, function()
            if (self.noncheatspot != nil and self.noncheatspot:Distance(self.lastTPspot) > 50) then -- Make sure they aren't being teleported where they last teleported at
                ...
                if (#player.GetAllPlayingAndAlive() > 1) then -- To the other player
                    ...
                    for k,v in pairs(ents.GetAll()) do -- To spawn
                        if v:GetClass() == "player_spawns" then   
                            self:SetPos(v:GetPos())
                            break
                        end
                    end
...
hook.Add("PlayerTick", "NZAntiCheat", function(ply)
