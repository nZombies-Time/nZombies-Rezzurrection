nzTools:CreateTool("zaddspawn2", {
	displayname = "Additional Spawn Creator 2",
	...
	PrimaryAttack = function(wep, ply, tr, data)
		...
		local ent
		if IsValid(tr.Entity) and tr.Entity:GetClass() == "nz_spawn_zombie_extra2" then
			ent = tr.Entity -- No need to recreate if we shot an already existing one
		else
			ent = nzMapping:ZedExtraSpawn2(tr.HitPos,(Angle(0,(ply:GetPos() - tr.HitPos):Angle()[2],0)), nil, tobool(data.master), data.spawntype, data.zombietype, data.roundactive,ply)
		end
	...
	displayname = "Extra Zombie Spawn Creator Type 2",
	desc = "LMB: Place Spawnpoint, RMB: Remove Spawnpoint",
	icon = "icon16/flag_pink.png",
	weight = 2,
	condition = function(wep, ply)
		return true
	end,
	interface = function(frame, data)
		...
		function DProperties.UpdateData(data)
			nzTools:SendData(data, "zaddspawn2")
		end
		...
		text:SetText("Special Spawnpoints apply to the enemy you have in extra slot 2")
