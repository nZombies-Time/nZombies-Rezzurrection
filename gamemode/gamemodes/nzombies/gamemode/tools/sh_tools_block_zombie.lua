...
nzTools:CreateTool("zombieblock", {
	displayname = "Invisible Zombie Block Spawner",
	...
	SecondaryAttack = function(wep, ply, tr, data)
		if IsValid(tr.Entity) and tr.Entity:GetClass() == "wall_block_zombie" then
			tr.Entity:Remove()
		end
	end,
	...
	interface = function(frame, data, context)
		...
		function Scroll.UpdateData(data)
			nzTools:SendData(data, "zombieblock", data) -- Save the same data here
		end
...
nzTools:EnableProperties("block", "Edit Model...", "icon16/brick_edit.png", 9009, true, function( self, ent, ply )
	...
	if ( ent:GetClass() != "wall_block_zombie" ) then return false end
