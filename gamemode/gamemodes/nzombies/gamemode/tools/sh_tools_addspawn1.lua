nzTools:CreateTool("zaddspawn1", {
	displayname = "Additional Spawn Creator",
	...
	PrimaryAttack = function(wep, ply, tr, data)
		...
		local ent
		if IsValid(tr.Entity) and tr.Entity:GetClass() == "nz_spawn_zombie_extra1" then
			ent = tr.Entity -- No need to recreate if we shot an already existing one
		else
			ent = nzMapping:ZedExtraSpawn1(tr.HitPos,(Angle(0,(ply:GetPos() - tr.HitPos):Angle()[2],0)), nil, tobool(data.master), data.spawntype, data.zombietype, data.roundactive,ply)
		end
	...
	displayname = "Extra Zombie Spawn Creator",
	desc = "LMB: Place Spawnpoint, RMB: Remove Spawnpoint",
	...
	interface = function(frame, data)
		...
		function DProperties.UpdateData(data)
			nzTools:SendData(data, "zaddspawn1")
		end
		...
		text:SetText("Special Spawnpoints apply to the enemy you have in extra slot 1")
