...
if SERVER then
	util.AddNetworkString("nz_InvisWallZombieCreation")
	
	net.Receive("nz_InvisWallZombieCreation", function(len, ply)
		if !ply:IsInCreative() then return end
		local vec1 = net.ReadVector()
		local vec2 = net.ReadVector()
		
		if net.ReadBool() then
			nzMapping:CreateInvisibleWallZombie(vec1, vec2, ply)
		else
...
nzTools:CreateTool("inviswall_zombie", {
	displayname = "Invisible Zombie Wall Creator",
	desc = "LMB: Set Corners, RMB: Remove Invisible Wall Block at spot, R: Reset corners",
	...
	SecondaryAttack = function(wep, ply, tr, data)
		...
		for k,v in pairs(walls) do
			if v:GetClass() == "invis_wall_zombie" then v:Remove() end
		end
	end,
	...
	PrimaryAttack = function(wep, ply, tr, data)
		...
		elseif !height then
			...
			net.Start("nz_InvisWallZombieCreation")
