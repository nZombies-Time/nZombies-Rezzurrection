nzTools:CreateTool("zaddspawn4", {
	...
	PrimaryAttack = function(wep, ply, tr, data)
		...
		local ent
		if IsValid(tr.Entity) and tr.Entity:GetClass() == "nz_spawn_zombie_extra4" then
			ent = tr.Entity -- No need to recreate if we shot an already existing one
		else
			ent = nzMapping:ZedExtraSpawn4(tr.HitPos,(Angle(0,(ply:GetPos() - tr.HitPos):Angle()[2],0)), nil, tobool(data.master), data.spawntype, data.zombietype, data.roundactive,ply)
		end
