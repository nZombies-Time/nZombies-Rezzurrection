...
local function GunHud_t7()
	...
	surface.SetDrawColor(ColorAlpha(color_white, math.Round(CurTime())%math.random(100, 255)))
	...
	if IsValid(wep) then
		...
		if class == "nz_multi_tool" then
			draw.SimpleTextOutlined(nzTools.ToolData[wep.ToolMode].displayname or wep.ToolMode, "nz.ammo.bo3.wepname", w - 230*scale, h - 80*scale, fontColor, TEXT_ALIGN_RIGHT, TEXT_ALIGN_BOTTOM, 2, color_black_50)
			draw.SimpleTextOutlined(nzTools.ToolData[wep.ToolMode].desc or "", "nz.ammo.bo3.main", w - 220*scale, h - 180*scale, color_t7, TEXT_ALIGN_RIGHT, TEXT_ALIGN_TOP, 4, color_t7_outline)
		elseif illegalspecials[wep.NZSpecialCategory] then
			draw.SimpleTextOutlined(wep:GetPrintName(), "nz.ammo.bo3.wepname", w - 230*scale, h - 80*scale, fontColor, TEXT_ALIGN_RIGHT, TEXT_ALIGN_BOTTOM, 2, color_black_50)
		elseif not illegalspecials[wep.NZSpecialCategory] then
			if wep.Primary then
				...
				if clip and clip > 0 then
					...
					if ammoType == -1 then
						...
						draw.SimpleTextOutlined(clip1, "nz.ammo.bo3.big", w - (300 - xlipoffset)*scale, h - 130*scale, ammoCol, TEXT_ALIGN_RIGHT, TEXT_ALIGN_BOTTOM, 4, outlineCol)
					else
						draw.SimpleTextOutlined(clip1, "nz.ammo.bo3.big", w - 300*scale, h - 130*scale, ammoCol, TEXT_ALIGN_RIGHT, TEXT_ALIGN_BOTTOM, 4, outlineCol)
						if resclip and resclip > 0 then
							draw.SimpleTextOutlined(ammoTotal, "nz.ammo.bo3.main", w - 220*scale, h - 120*scale, color_t7_ammo, TEXT_ALIGN_RIGHT, TEXT_ALIGN_BOTTOM, 4, color_t7_outline)
						end
					end
				else
					...
					if resclip and resclip > 0 then
						...
						draw.SimpleTextOutlined(ammoTotal, "nz.ammo.bo3.big", w - (295 - ammooffset)*scale, h - 130*scale, reserveCol, TEXT_ALIGN_RIGHT, TEXT_ALIGN_BOTTOM, 4, reserveoutCol)
					end
				end
			end
			...
			if wep.Secondary and (not wep.CanBeSilenced or (wep.CanBeSilenced and not wep:GetSilenced() and wep.Clip3)) then
				...
				if clip2 and clip2 > 0 then
					...
					draw.SimpleTextOutlined(wep:Clip2(), "nz.ammo.bo3.big", w - 385*scale, h - 130*scale, ammoCol, TEXT_ALIGN_RIGHT, TEXT_ALIGN_BOTTOM, 4, outlineCol)
				else
					if ammoTotal2 and ammoTotal2 > 0 then
						...
						draw.SimpleTextOutlined(ammoTotal2, "nz.ammo.bo3.big", w - 385*scale, h - 130*scale, reserveCol, TEXT_ALIGN_RIGHT, TEXT_ALIGN_BOTTOM, 4, reserveoutCol)
					end
				end
			end
			...
			/*if nz_showgun:GetBool() and killicon.Exists(class) then
				surface.SetFont("nz.ammo.bo3.wepname")
...
local function PerksMMOHud_t7()
	...
	for k, v in pairs(ply:GetPerks()) do
		...
		elseif mmohud.style == "count" then
			draw.SimpleTextOutlined(ply:GetNW2Int(tostring(mmohud.count), 0), ChatFont, w - 285*scale - (40*traycount*scale), h - 205*scale, fontColor, TEXT_ALIGN_RIGHT, TEXT_ALIGN_BOTTOM, 1, color_black)
		elseif mmohud.style == "%" then
			...
			if (not mmohud.hide) or (mmohud.hide and perkpercent < 100) then
				draw.SimpleTextOutlined(perkpercent.."%", ChatFont, w - 285*scale - (40*traycount*scale), h - 205*scale, fontColor, TEXT_ALIGN_RIGHT, TEXT_ALIGN_BOTTOM, 1, color_black)
			end
		elseif mmohud.style == "chance" then
			draw.SimpleTextOutlined(ply:GetNW2Int(tostring(mmohud.count), 0).."/"..mmohud.max, ChatFont, w - 285*scale - (40*traycount*scale), h - 205*scale, fontColor, TEXT_ALIGN_RIGHT, TEXT_ALIGN_BOTTOM, 1, color_black)
		end
	...
	for i, ent in nzLevel.GetHudEntityArray() do
		...
		if (ent.NZThrowIcon or ent.NZNadeRethrow) and (ent:GetCreationTime() + 0.3 < CurTime()) then
			...
			if data.visible then
				...
				surface.SetMaterial(ent.NZThrowIcon or t7_icon_grenade_hud)
...
local function PerksHud_t7()
	...
	if perkflashtime > CurTime() then
		...
		cwimage("rf/round/sparks/" .. (math.ceil(CurTime()*30) % 20) .. ".png", (w + 46*pscale) + ((flashcount-1)*(size + 6))*pscale, h - 154*pscale - 64*flashrow, 168*pscale, 252*pscale, ColorAlpha(color_t7_sparks, 255*alpha))
