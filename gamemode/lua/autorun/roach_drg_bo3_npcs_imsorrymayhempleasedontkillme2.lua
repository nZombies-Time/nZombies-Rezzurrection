
game.AddParticles("particles/bo1overhaul_blood_fx.pcf")
	PrecacheParticleSystem("nz_blood_gib")
	PrecacheParticleSystem("nz_blood_headshot")
game.AddParticles("particles/driese_fx.pcf")
	PrecacheParticleSystem("driese_tp_arrival_phase1")
game.AddParticles("particles/hound.pcf")
game.AddParticles("particles/blackops3zombies_fx.pcf")
	PrecacheParticleSystem("bo3_astronaut_incoming")
	PrecacheParticleSystem("bo3_astronaut_pulse")
	
	PrecacheParticleSystem("bo3_hellhound_aura")
	PrecacheParticleSystem("hound_explosion")
	PrecacheParticleSystem("hound_summon")
	
	PrecacheParticleSystem("bo3_mangler_blast")
	PrecacheParticleSystem("bo3_mangler_charge")
	PrecacheParticleSystem("bo3_mangler_pulse")
	
	PrecacheParticleSystem("bo3_margwa_death")
	PrecacheParticleSystem("bo3_margwa_slam")
	
	PrecacheParticleSystem("bo3_n6crawler_aura")
	
	PrecacheParticleSystem("bo3_napalm_explosion")
	PrecacheParticleSystem("bo3_napalm_fs")
	
	PrecacheParticleSystem("bo3_panzer_elec_blast")
	PrecacheParticleSystem("bo3_panzer_elec_nade")
	PrecacheParticleSystem("bo3_panzer_engine")
	PrecacheParticleSystem("bo3_panzer_explosion")
	PrecacheParticleSystem("bo3_panzer_flame")
	PrecacheParticleSystem("bo3_panzer_landing")
	
	PrecacheParticleSystem("bo3_shrieker_scream")
	
	PrecacheParticleSystem("bo3_spider_impact")
	PrecacheParticleSystem("bo3_spider_projectile")
	PrecacheParticleSystem("bo3_spider_spit")
	
	PrecacheParticleSystem("bo3_thrasher_aura")
	PrecacheParticleSystem("bo3_thrasher_blood")
	
	PrecacheParticleSystem("bo3_zombie_spawn")
	game.AddParticles("particles/sentinel_laser.pcf")
	PrecacheParticleSystem("sentinel_laser")