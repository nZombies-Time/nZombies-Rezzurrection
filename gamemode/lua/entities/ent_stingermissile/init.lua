...
function ENT:Initialize()
	self.missiletime = CurTime() + 60
	self.Entity:SetModel("models/missiles/stinger_missile_mw2.mdl")
	self.Entity:SetMaterial("models/jessev92/weapons/hl2/stinger/stinger")
...
function ENT:FireMotor()
	...
	self.Entity:EmitSound("stinger/ignite.wav")
...
function ENT:Think()
	...
	effdat:SetScale( 1 )
	util.SpriteTrail( self, 0, Color( 255, 255, 255 ), false, 10, 0, 1, 1/(15+5)*0.5, "trails/smoke.vmt")
	...
	if(IsValid(self.Target))then
		...
		if((IsValid(self.Target))and(self.Engaged))then
			...
			if(TargetPosition)then 
				...
				--end
				if(Dist<50) and !self.Target:IsNPC() then
					self.Target = nil
				end
	...
	self.MotorPower=self.MotorPower+500
	if(self.MotorPower>=500)then self.MotorPower=500 end
...
function ENT:Detonate()
	...
	if(true)then
		...
		util.BlastDamage(self.Entity,self.HLOwner,SelfPos,350,500)
		local Boom=EffectData()
		Boom:SetOrigin(SelfPos)
		Boom:SetScale(1)
