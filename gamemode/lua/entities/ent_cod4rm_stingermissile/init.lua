...
function ENT:Initialize()
	...
	self.Entity:SetModel("models/missiles/stinger_missile_cod4rm.mdl")
	self.Entity:SetMaterial("models/krazy/cod4rm/cod4rm_stinger_missile")
...
function ENT:Think()
	...
	if(self.MotorPower>=1750)then self.MotorPower=1750 end
...
function ENT:Detonate()
	...
	if(true)then
		...
		for key,thing in pairs(ents.FindInSphere(SelfPos,500))do
				if(table.HasValue({"npc_strider","npc_combinegunship","npc_helicopter","npc_turret_floor","npc_turret_ground","npc_turret_ceiling","npc_apcdriver"},thing:GetClass()))then
		...
		util.BlastDamage(self.Entity,self,SelfPos,350,6000)
		local Boom=EffectData()
		Boom:SetOrigin(SelfPos)
		Boom:SetScale(1)
