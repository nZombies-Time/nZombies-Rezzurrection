...
SWEP.Category				= "Krazy TFA Weapons" --The category.  Please, just choose something generic or something I've already done if you plan on only doing like one swep..
...
SWEP.ViewModel			= "models/krazy/mw2/stinger_v.mdl" --Viewmodel path
...
--}
--[[WORLDMODEL]]--
SWEP.WorldModel			= "models/jessev92/weapons/hl2/stinger_w.mdl" -- Weapon world model path
...
SWEP.IronSightsPos = Vector(0.006, -4.581, 1.97)
SWEP.IronSightsAng = Vector(7.296, -21.063, 32.264)
SWEP.IronSightTime = 1
--[[INSPECTION]]--
SWEP.InspectPos = Vector(6.519, -3.289, 2.19)
SWEP.InspectAng = Vector(32.361, 25.198, 44.375)
...
if SERVER then
	if IsValid(self.LockedEnt) then
		...
		self.Owner:StopSound( "lockedon" )
		self.Owner:EmitSound("stinger/rocketfire.wav")
		self.LockedEnt = nil
		tempent = nil
...
function SWEP:FireMissal()
	...
	--fire round
	local Miss=ents.Create("ent_stingermissile")
...
function SWEP:FireShot()
		...
		local Spred=self.ShotSpread
		--fire round
		local Miss=ents.Create("ent_homingmissile_unguided")
		local Ang=Dir:Angle()
		local angl = self:EyeAngles()
		local aim = self.Owner:GetAimVector()
		Miss.HLOwner = self.Owner
...
function SWEP:Think()
	...
	if self.StripTimer < CurTime() then
		self.Dispose = true
		if SERVER then
			self.Owner:StripWeapon("mw2_fim92stinger")
		end
	end
end
	...
	if IsValid(tr.Entity) and self.Weapon:Clip1() >= 1 and !self.Owner:KeyDown(IN_SPEED) then
		if tr.Entity:IsNPC() or tr.Entity:IsPlayer() or tr.Entity:IsVehicle() or tr.Entity:IsValid()  then
			if self.LOCKD < 3 and self.BeepDelay <= CurTime() then
				if self.LOCKD != 2 then
					self.Owner:EmitSound("stinger/check.wav")
				end
				tempent = tr.Entity
				self.BeepDelay = CurTime() + 0.7	
				self.LOCKD = self.LOCKD + 1
				print (self.LOCKD)
				--if CLIENT or SERVER and game.SinglePlayer() and self.LOCKD <= 1 then
			end
			if self.LOCKD == 3 then
				self.BeepDelay = CurTime() + 0.7	
				self.LOCKD = self.LOCKD + 1	
				self.LockedEnt = tr.Entity
				self.Owner:EmitSound("lockedon")
				local TargetPos=self.LockedEnt:GetPos()
